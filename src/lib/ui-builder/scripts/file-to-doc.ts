...
export function fileToDoc(filePath: string): ComponentDocWithIsDefault[] {
  ...
  return docs.map((doc) => {
    const exportInfo = exportNames.find((exp) => {
      return (
        exp.displayName === doc.displayName ||
        exp.name.toLowerCase() === doc.displayName.toLowerCase()
      );
    });
  ...
  defaultExports.forEach((exportAssignment) => {
    ...
    if (expr) {
      ...
      if (Node.isIdentifier(expr)) {
        ...
        const identifierArg = args.find((arg) => Node.isIdentifier(arg)) as
          | Identifier
          | undefined;
